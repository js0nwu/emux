...
def find_objects_cascade(picture, detector):
    ...
    return [dlib.rectangle(left=int(f[0]), top=int(f[1]), right=int(f[0] + f[2]), bottom=int(f[1] + f[3])) for f in faces]
...
def warp_picture(picture, matrix, shape):
    ...
    cv2.warpAffine(picture, matrix[:2], (shape[1], shape[0]), dst = output, borderMode = cv2.BORDER_TRANSPARENT, flags = cv2.WARP_INVERSE_MAP)
    return output
...
def color_correct(a, b, la):
    blur_amount = COLOR_CORRECT_BLUR * numpy.linalg.norm(numpy.mean(la[LEFT_EYE_POINTS], axis=0) - numpy.mean(la[RIGHT_EYE_POINTS], axis=0))
...
def face_swap(mat_picture, mat_replace):
    ...
    return (linear_blend_mask(mat_picture, color_correct(mat_picture, mat_replace, f_landmarks), mask), f_landmarks, r_landmarks, mask)
